...
1. **Stage 1: Multi-concept DreamBooth Training** (Step 1 in `run.sh`)
   ...
   - Produces checkpoints saved in:
     - `{EXP_DIR}/unet/{step}/` - UNet model weights
     - `{EXP_DIR}/text_encoder/{step}/` - Text encoder weights
   - Default training: 1000 steps (phase1) + 4000 steps (phase2)
   - Checkpoints saved every 1000 steps after phase 1
