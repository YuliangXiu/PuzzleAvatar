...
def get_clip_token_for_string(tokenizer, string):
    ...
    assert torch.count_nonzero(
        tokens - 49407
    ) == 2, f"String '{string}' maps to more than a single token. Please use another string"
...
def get_bert_token_for_string(tokenizer, string):
    ...
    assert torch.count_nonzero(
        token
    ) == 3, f"String '{string}' maps to more than a single token. Please use another string"
...
class EmbeddingManager(nn.Module):
    ...
    def save(self, ckpt_path):
        torch.save({
            "string_to_token": self.string_to_token_dict, "string_to_param":
