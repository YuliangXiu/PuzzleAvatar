...
def parse_args():
    ...
    parser.add_argument(
        "--do_not_use_full_shot",
        action="store_false",
        help="Use full shot human images",
        dest="use_full_shot"
    )
    parser.add_argument(
        "--use_half_data",
        action="store_true",
        help="Use half of human images",
        dest="use_half_data"
    )
...
def parse_args_multi():
    ...
    subject_outfit_lst = args.project_name.split("_")
    ...
    for (subject, outfit) in zip(subject_outfit_lst[0::2], subject_outfit_lst[1::2]):
        
        if subject == 'human':
            with open(
                os.path.join(f"./data/{subject}/{outfit}", 'gpt4v_simple.json'),
                'r'
            ) as f:
        ...
        so_keys = list(gpt4v_response[f"{subject}_{outfit}"].keys())
        gpt4v_classes_lst.append(so_keys)
        so_map_lst.append([f"{subject}_{outfit}"] * len(so_keys))
        gender_lst.append(['man'] *
                          len(so_keys) if gpt4v_response[f"{subject}_{outfit}"]['gender'] in
                          ['man', 'male'] else ['woman'] * len(so_keys))

    for so_idx in range(len(gpt4v_classes_lst)):
        for key in ["gender", "eyeglasses", "sunglasses", "glasses"]:
            if key in gpt4v_classes_lst[so_idx]:
                key_idx = gpt4v_classes_lst[so_idx].index(key)
                gpt4v_classes_lst[so_idx].pop(key_idx)
                so_map_lst[so_idx].pop(key_idx)
                gender_lst[so_idx].pop(key_idx)

    args.gender = flatten(gender_lst)
    args.initializer_tokens = flatten(gpt4v_classes_lst)
    args.num_of_assets = len(args.initializer_tokens)
    ...
    return args, gpt4v_response, flatten(so_map_lst)
