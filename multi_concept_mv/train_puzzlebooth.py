...
import multi_concepts.ptp_utils as ptp_utils
from multi_concepts.ptp_utils import AttentionStore
from mvdream_diffusers.mv_unet import MultiViewUNetModel
...
def parse_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=1993, help="A seed for reproducible training."
    )
    parser.add_argument(
        "--resolution",
        type=int,
    ...
    parser.add_argument(
        "--boft_bias_fit", action="store_true", help="Whether to use bias fit"
    )
    parser.add_argument(
        "--boft_dropout",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="Bias type for BOFT. Can be 'none', 'all' or 'boft_only', only used if use_peft is 'boft'",
    )
    parser.add_argument(
        "--lora_r",
        type=int,
    ...
    assert (
        len(args.initializer_tokens) == 0
        or len(args.initializer_tokens) == args.num_of_assets
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        instance_img_paths = sorted(
            glob.glob(f"{instance_data_root}/image/*[!raw].jpg")
        )
        ...
        for instance_img_path in instance_img_paths:
            ...
            instance_mask_paths = glob.glob(
                f"{instance_data_root}/mask/{instance_idx}_*.png"
            )
        ...
        if self.class_data_root is not None and self.with_prior_preservation:
            ...
            self.class_images_path[self.gender] = [
                item
                for item in sorted(list(class_rgb_dir.iterdir()))
            ...
            self.syn_images_path[self.gender] = [
                item
                for item in sorted(list(syn_rgb_dir.iterdir()))
                if str(item).endswith(".png")
            ]
            self.syn_normals_path[self.gender] = [
                item
                for item in sorted(list(syn_normal_dir.iterdir()))
                if str(item).endswith(".png")
            ]
            self.syn_json_path[self.gender] = [
                item
                for item in sorted(list(syn_json_dir.iterdir()))
        ...
        self._length = max(
            len(self.instance_images), self.num_class_images, self.num_syn_images
        )
    ...
    def construct_prompt(
        ...
        if is_face:
            prompt_head = (
                f"a high-resolution DSLR colored image of the face of a {self.gender}"
            )
        else:
        ...
        for class_token in with_classes:
            ...
            if (
                len(wear_classes) == 0
                and with_classes.index(class_token) == len(with_classes) - 1
            ):
            ...
            if self.use_shape_desc:
                prompt += (
                    f"{tokens_to_use[idx]} {descs_to_use[idx]} {class_token}{ending}"
                )
    ...
    def __getitem__(self, index):
        ...
        num_of_tokens = random.randrange(
            1, len(self.placeholder_tokens[index % example_len]) + 1
        )
        ...
        for attn_cls in ["face"]:
            if attn_cls in self.class_tokens[index % example_len]:
                sample_prop[self.class_tokens[index % example_len].index(attn_cls)] = (
                    3.0
                )
        ...
        tokens_to_use = [
            self.placeholder_tokens[index % example_len][tkn_i]
        ...
        descs_to_use = [
            self.instance_descriptions[index % example_len][tkn_i]
            for tkn_i in tokens_ids_to_use
        ]
        ...
        example["instance_masks"] = self.instance_masks[index % example_len][
            tokens_ids_to_use
        ]
        ...
        if self.class_data_root and self.with_prior_preservation:
            cls_img_path = self.class_images_path[self.gender][
                index % self.num_class_images
            ]
            syn_img_path = self.syn_images_path[self.gender][
                index % self.num_syn_images
            ]
            syn_norm_path = self.syn_normals_path[self.gender][
                index % self.num_syn_images
            ]
            ...
            example["syn_prompt_ids_raw"] = self.tokener(
                self.construct_prompt(syn_classes, syn_tokens, syn_descs, [], is_face)[
                    "prompt_raw"
                ]
            )
            ...
            example["person_filename"] = self.tokener(
                os.path.basename(str(cls_img_path))
            )
...
def bbox2(img):
    ...
    c_width = cmax - cmin
    rmin, rmax = (
        max(0, rmin - r_height // 4),
        min(img.shape[0] - 1, rmax + r_height // 4),
    )
...
def padded_stack(
    ...
    out = torch.stack(
        [
            F.pad(x, make_padding(full_size - x.size(dim)), mode=mode, value=value)
            if full_size - x.size(dim) > 0
            else x
            for x in tensors
        ],
        dim=0,
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    if with_prior_preservation:
        ...
        pixel_values = (
            person_pixel_values + syn_pixel_values + syn_norm_values + pixel_values
        )

        batch["person_filenames"] = torch.cat(
            [example["person_filename"] for example in examples], dim=0
        )
...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        # print("attention processor after init", self.unet.attn_processors.keys())
        ...
        self.class_ids = self.tokenizer(
            " ".join(self.args.initializer_tokens),
        ).input_ids[1:-1]
        ...
        cached_masks_path = Path(
            ...
            f"{pretrained_name}/{self.args.gender}/attn_masks_cache.pt",
        )
                ...
                if cached_masks_path.exists():
                    ...
                    for class_name in [self.args.gender]:
                        ...
                        if cur_class_images < self.args.num_class_images:
                            num_new_images = (
                                self.args.num_class_images - cur_class_images
                            )
                            logger.info(
                            ...
                            sample_dataloader = self.accelerator.prepare(
                                sample_dataloader
                            )
                            ...
                            for example in tqdm(
                                ...
                                # print(example["prompt"], negative_prompt)
                                # images = pipeline(
                                #     example["prompt"],
                                ...
                                #     output_type='pil',
                                # )
                                # print('attention before forward', self.controller.attention_store.keys())
                                images = pipeline(
                                    example["prompt"][0],
                                    negative_prompt=negative_prompt,
                                    num_frames=1,
                                ...
                                for batch_idx in range(self.args.sample_batch_size):
                                    ...
                                    for class_token_id in self.class_ids:
                                        ...
                                        cls_attn_mask = (
                                            cls_attn_mask / cls_attn_mask.max()
                                        )
                                        self.attn_mask_cache[base_name][
                                            class_token_id
                                        ] = cls_attn_mask
        ...
        prompt_dict = train_dataset.construct_prompt(
            ...
            self.placeholder_tokens,
            [
                self.gpt4v_response[cls_name]
                for cls_name in self.args.initializer_tokens
            ],
            [],
        )
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                    ...
                    if self.args.train_text_encoder:
                        if self.args.use_peft == "none":
                            ...
                            self.text_encoder = get_peft_model(
                                self.text_encoder, text_config
                            )
                            self.text_encoder.to(self.accelerator.device)
                            self.text_encoder.print_trainable_parameters()

                            logger.info(
                                "***** Training with BOFT fine-tuning (text_encoder) *****"
                            )
                            # logger.info(
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    if self.args.lambda_attention != 0:
                        ...
                        for batch_idx in range(self.args.train_batch_size):
                            ...
                            if self.args.with_prior_preservation:
                                curr_cond_batch_idx = (
                                    self.args.train_batch_size * (bsz - 1) + batch_idx
                                )
                            ...
                            for mask_id in range(
                                len(batch["class_ids"][batch_idx].nonzero())
                            ):
                                curr_placeholder_token_id = self.placeholder_token_ids[
                                    batch["token_ids"][batch_idx][mask_id]
                                ]
                                curr_class_token_id = batch["class_ids"][batch_idx][
                                    mask_id
                                ]
                                ...
                                if self.args.with_prior_preservation:
                                    ...
                                    if self.args.apply_masked_prior:
                                        ...
                                        if (
                                            ...
                                            not in self.attn_mask_cache[
                                                person_filename
                                            ].keys()
                                        ):
                                            self.attn_mask_cache[person_filename][
                                                curr_class_token_id.item()
                                            ] = cls_attn_mask.detach()
                        ...
                        if self.args.with_prior_preservation:
                            cls_mask = F.interpolate(
                                torch.stack(cls_masks),
                                size=(self.mask_res, self.mask_res),
                            )
                            ...
                            syn_norm_rgb_loss = (
                                F.mse_loss(
                                    syn_norm_pred.float()
                                    * downsampled_syn_mask.float(),
                                    syn_norm_target.float()
                                    * downsampled_syn_mask.float(),
                                    reduction="mean",
                                )
                                * self.args.syn_loss_weight
                            ...
                            logs["l_syn"] = (
                                syn_rgb_loss.detach().item()
                                + syn_norm_rgb_loss.detach().item()
                            )

                            if self.args.apply_masked_prior:
                                cls_mask_gt = F.interpolate(
                                    torch.stack(cls_masks_gt),
                                    size=(self.mask_res, self.mask_res),
                                )
                    ...
                    if global_step < self.args.phase1_train_steps:
                        ...
                        with torch.no_grad():
                            ...
                            ] = orig_embeds_params[: -self.args.num_of_assets]
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        global_step % self.args.checkpointing_steps == 0
                    ...
                    if (
                        ...
                        if self.args.lambda_attention != 0:
                            ...
                            last_sentence = clean_decode_prompt(
                                last_sentence, self.tokenizer
                            )

                            step_attn_vis = self.save_cross_attention_vis(
                                last_sentence,
                                batch_pixels=batch["pixel_values"][curr_cond_batch_idx]
                                .detach()
                                .cpu(),
                            ...
                            self.accelerator.trackers[0].log(
                                {
                                    "step_attn": [
                                        wandb.Image(
                                            step_attn_vis, caption=f"{last_sentence}"
                                        )
                                    ]
                                }
                            )
                            ...
                            if self.args.with_prior_preservation:
                                ...
                                prior_attn_vis = self.save_cross_attention_vis(
                                    last_sentence,
                                    batch_pixels=batch["pixel_values"][batch_idx]
                                    .detach()
                                    .cpu(),
                                    attention_maps=agg_attn_prior.detach().cpu(),
                                    path=os.path.join(
                                        img_logs_path,
                                        f"{global_step:05}_step_prior_attn.jpg",
                                    ),
                                )
                        ...
                        for idx, name in enumerate(["", "_raw"]):
                            cur_prompt = (
                                ...
                                if name == ""
                                else self.args.instance_prompt_raw
                            )
                            ...
                            full_attn_vis = self.save_cross_attention_vis(
                                ...
                                path=os.path.join(
                                    img_logs_path,
                                    f"{global_step:05}_full_attn{name}.jpg",
                                ),
                            )
                            vis_dict.update(
                                {
                                    f"full_attn{name}": [
                                        wandb.Image(
                                            full_attn_vis, caption=f"{cur_prompt}"
                                        )
                                    ]
                                }
                            )
            ...
            for filename in existed_masks.keys():
                for mask_id in existed_masks[filename].keys():
                    if mask_id not in self.attn_mask_cache[filename].keys():
                        self.attn_mask_cache[filename][mask_id] = existed_masks[
                            filename
                        ][mask_id]
            torch.save(self.attn_mask_cache, cached_masks_path)
    ...
    def save_adaptor(self, step=""):
        if self.args.use_peft != "none":
            ...
            logger.info(
                f"Saved unet to {os.path.join(self.args.output_dir, f'unet/{step}')}"
            )

            if self.args.train_text_encoder:
                unwarpped_text_encoder = self.accelerator.unwrap_model(
                    self.text_encoder
                )
    ...
    def register_attention_control(self, controller):
        ...
        with open(osp.basename(self.args.pretrained_model_name_or_path) + ".txt", "w") as f:
            ...
            f.write("-----------\n")
            # write all models
            f.write(str(self.unet) + "\n")

        if "mvdream" in self.args.pretrained_model_name_or_path:
            mid_key = 'middle_block'
            up_key = 'output_blocks'
            down_key = 'input_blocks'
        else:
            mid_key = 'mid_block'
            up_key = 'up_blocks'
        ...
        for name in self.unet.attn_processors.keys():
            ...
            elif name.startswith(up_key):
                place_in_unet = "up"
            elif name.startswith(down_key):
    ...
    def perform_full_inference(self, guidance_scale=7.5):
        ...
        latents = torch.randn(
            (2, 4, self.mask_res, self.mask_res), device=self.accelerator.device
        )
        ...
        self.accelerator.trackers[0].log(
            {
                "validation": [
                    wandb.Image(images[0], caption=f"{self.args.instance_prompt}")
                ],
                "validation_raw": [
                    wandb.Image(images[1], caption=f"{self.args.instance_prompt_raw}")
                ],
            }
        )
