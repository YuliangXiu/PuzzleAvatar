...
class DMTet(nn.Module):
    def __init__(self):
        ...
        num_triangles_table = torch.tensor([0, 1, 1, 2, 1, 2, 2, 1, 1, 2, 2, 1, 2, 1, 1, 0],
                                           dtype=torch.long)
    ...
    def map_uv(self, faces, face_gidx, max_idx):
        ...
        uvs = torch.stack([
            tex_x, tex_y, tex_x + pad, tex_y, tex_x + pad, tex_y + pad, tex_x, tex_y + pad
        ],
                          dim=-1).view(-1, 2)
        ...
        uv_idx = torch.stack((tet_idx * 4, tet_idx * 4 + tri_idx + 1, tet_idx * 4 + tri_idx + 2),
                             dim=-1).view(-1, 3)
    ...
    def __call__(self, pos_nx3: Tensor, sdf_n: Tensor, tet_fx4: Tensor, with_uv: bool = True):
        ...
        face_gidx = torch.cat(
            (
                ...
                torch.stack(
                    (tet_gidx[num_triangles == 2] * 2, tet_gidx[num_triangles == 2] * 2 + 1),
                    dim=-1
                ).view(-1)
            ),
            dim=0,
