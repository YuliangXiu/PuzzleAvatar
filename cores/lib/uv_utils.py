...
def diffuse_color_with_mask(img_m, img_c, num_iter=1, ksize=3):
    ...
    #* expand
    img_m = cv2.copyMakeBorder(
        img_m, hksize, hksize, hksize, hksize, cv2.BORDER_CONSTANT, value=(0)
    )
    img_c = cv2.copyMakeBorder(
        img_c, hksize, hksize, hksize, hksize, cv2.BORDER_CONSTANT, value=(0, 0, 0)
    )
...
def texture_padding(img_c0, img_m0, fac=1.25):
    ...
    * reference:
    ...
    img_m0, img_c0 = diffuse_color_with_mask(
        img_m0, img_c0, 2
    )    #* diffuse 2 pixels (2x2 downsampling)

    img_m1 = img_m0.copy()
    img_c1 = img_c0.copy()
    while np.any(img_m1 == 0):
        img_m1 = cv2.resize(
            img_m1, (int(img_m1.shape[0] / fac), int(img_m1.shape[1] / fac)),
            interpolation=cv2.INTER_LINEAR
        )
        img_c1 = cv2.resize(
            img_c1, (int(img_c1.shape[0] / fac), int(img_c1.shape[1] / fac)),
            interpolation=cv2.INTER_LINEAR
        )
        img_m1[img_m1 != 255] = 0
        img_c1[img_m1 == 0] = 0
